...
class DeletePlanFeature(Endpoint):
    ...
    def delete_plan_feature(self, url_bill, headers, max_retries, wait_sec):
        ...
        for attempt in range(max_retries):
            try:
                self.response = requests.delete(f'{url_bill}/v3/plans/{plan_id}/content/{plan_feature_id}', headers=headers)
    ...
    def check_deletion(self, url_bill, headers):
        ...
        if self.response.status_code != 404:
            pytest.fail('Plan feature does not deleted')
        print('Plan feature deleted')
