...
class UpdateClient(Endpoint):
    ...
    def cancel_recurring_payments(self, url_bill, headers, payload, max_retries, wait_sec):
        ...
        for attempt in range(max_retries):
            try:
                self.response = requests.put(f'{url_bill}/v1/clients/{client_id}/options', headers=headers, json=payload)
                self.response.raise_for_status()
                print('Response Status Code:', self.response.status_code)
                break
    ...
    def delete_yoomoney(self, url_bill, headers, max_retries, wait_sec):
        ...
        for attempt in range(max_retries):
            try:
                self.response = requests.delete(f'{url_bill}/v1/payment_methods/21/allowed/{client_id}', headers=headers)
                self.response.raise_for_status()
                print('Response Status Code:', self.response.status_code)
                break
    ...
    def check_signup(self,url_iam, headers):
        client_id = get('client_id')
        ...
        if current_signup is not None:
            pytest.fail('Sign Up does not change')
        print('Sign Up updated')

    def check_recurring_payments(self, url_bill, headers):
        client_id = get('client_id')
    ...
    def check_payments_methods(self, url_bill, headers):
        client_id = get('client_id')
